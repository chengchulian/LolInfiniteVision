...
namespace method
{
    ...
    void RedireceConsole()
    {
        ...
        hConsole = CreateFile(L"CONOUT$", GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
    ...
    size_t ConvertStringToByteArray(const char* pattern, BYTE* outBuffer, size_t maxSize)
    {
        ...
        for (size_t i = 0; pattern[i] && count < maxSize; ++i)
        {
            ...
            {
                outBuffer[count++] = (HexCharToByte(pattern[i]) << 4) | HexCharToByte(pattern[i + 1]);
                ++i;
            }
        }
        return count;
    ...
    DWORD64 LocateSignature(HANDLE hProcess, const char* maskedPattern, DWORD64 start, DWORD64 end, int offset)
    {
        ...
        size_t patternSize = ConvertStringToByteArray(maskedPattern, pattern, MAX_PATTERN_SIZE);
        if (patternSize == 0) return 0;

        while (start < end)
        {
            ReadProcessMemory(hProcess, (LPCVOID)start, page, PAGE_SIZE, nullptr);
            for (size_t i = 0; i <= PAGE_SIZE - patternSize; ++i)
            {
    ...
    {
        return GetPrivateProfileInt(lpAppName, lpKeyName, nDefault, CONFIG_PATH);
    }

    DWORD GetStringPrivateProfile(LPCWSTR lpAppName, LPCWSTR lpKeyName, LPCWSTR lpDefault, LPWSTR lpReturnedString, DWORD nSize)
    {
        return GetPrivateProfileString(lpAppName, lpKeyName, lpDefault, lpReturnedString, nSize, CONFIG_PATH);
    }

    BOOL WriteStringPrivateProfile(LPCWSTR lpAppName, LPCWSTR lpKeyName, LPCWSTR lpString)
    {
        return WritePrivateProfileString(lpAppName, lpKeyName, lpString, CONFIG_PATH);
    }
